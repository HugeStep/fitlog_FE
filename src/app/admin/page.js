...
export default function Page() {
    ...
    const fetchExercises = async () => {
        setLoading(true);
        try {
            const res = await fetch("http://localhost:8080/api/admin/exercises", {
    ...
    const handleUpdate = async () => {
        ...
        try {
            const res = await fetch(`http://localhost:8080/api/admin/exercises/${selectedExercise.exerciseId}`, {
                method: "PUT",
                headers: {
                    "Content-Type": "application/json",
                    ...getAuthHeader(),
                ...
                <div className="p-6">
                    ...
                    <div className="overflow-x-auto border border-gray-200 rounded-lg">
                        <table className="min-w-full divide-y divide-gray-200 text-sm text-center">
                            ...
                            {loading ? (
                                <tr><td colSpan="6" className="py-10 text-gray-500">데이터를 불러오는 중...</td></tr>
                            ) : exercises.length === 0 ? (
                                <tr><td colSpan="6" className="py-10 text-gray-400">데이터가 없습니다. '전체 불러오기'를 눌러보세요.</td></tr>
                            ) : (
                                exercises.map((ex) => (
                                    <tr key={ex.exerciseId} className="hover:bg-gray-50">
                                        ...
                                        <td className="px-4 py-3 text-blue-600 bg-blue-50 rounded-full text-xs inline-block my-2 px-2 py-1">{ex.unitType}</td>
                        ...
                        {activeModal === "create" && (
                            <>
                                ...
                                <div className="space-y-4">
                                    <div>
                                        <label className="block text-sm font-medium text-gray-700 mb-1">운동 이름 (name)</label>
                                        <input
                                            type="text"
                                    ...
                                    <div>
                                        <label className="block text-sm font-medium text-gray-700 mb-1">운동 카테고리 id (categoryId)</label>
                                        <input
                                            type="number"
                                            className="w-full border rounded px-3 py-2 focus:ring-2 focus:ring-blue-500 outline-none"
                                    ...
                                    <div>
                                        <label className="block text-sm font-medium text-gray-700 mb-1">운동 타입 (unitType)</label>
                        ...
                        {activeModal === "delete" && (
                            <>
                                ...
                                <div className="text-center py-6">
                                    <p className="text-lg text-gray-800 font-medium mb-8">정말 삭제하시겠습니까?</p>
                                    <button
